...
export default async function PayeePage() {
  ...
  if (!payee.personId) {
    ...
    return <main className="payee-page"><header className="payee-topbar"><div className="payee-brand"><Image src="/villix-logo.svg" alt="Villix" width={44} height={34} style={{ width: 44, height: 34 }} priority unoptimized /><span>Villix Contributor</span></div><a href={contributorPortalPath(host, "signout")}>Sign out</a></header><div className="payee-content"><section className="payee-welcome"><span>Complete your profile</span><h1>Welcome, {application.first_name}.</h1><p>Your team selection is already saved. Add the Shipd.ai username you use now or plan to create to finish your Villix contributor profile.</p></section><section className="payee-status-card"><ShipdHandleForm firstSetup /></section><footer className="payee-footer"><span>Signed in as {application.email}</span><span>You can return later to update your username.</span></footer></div></main>;
  }
  ...
  const paymentRoute = paidThroughLead ? "Team leader" : person.role === "team_lead" ? "Team lead" : "Direct";
  const paymentRouteDescription = paidThroughLead
    ? "Included in your team leader’s payout"
    : person.role === "team_lead"
      ? "Your eligible team total is paid to you"
      : "Paid to your verified account";
  ...
  return <main className="payee-page">
    ...
    <div className="payee-content"><section className="payee-welcome"><span>Recipient workspace</span><h1>Welcome, {person.display_name}.</h1><p>Manage your payout readiness and follow every Villix payment without entering the administrator workspace.</p></section>
      ...
      <div className="payee-details-grid">
        <section className="payee-history shipd-profile-card"><div className="payee-section-heading"><div><span>Profile details</span><h2>Shipd.ai account</h2></div><small>{person.shipd_handle_status === "matched" ? "Saved" : "Editable"}</small></div><ShipdHandleForm initialHandle={person.handle} locked={person.shipd_handle_status === "matched"} /></section>
        <section className="payee-overview-card">
          <div className="payee-overview-heading"><div><span>Payout overview</span><h2>Your earnings</h2></div><small>INR</small></div>
          <div className="payee-overview-total"><span>Total paid</span><strong>{inr.format(paidTotal)}</strong><small>Confirmed Villix payouts</small></div>
          <div className="payee-overview-meta"><div><span>Payment route</span><strong>{paymentRoute}</strong><small>{paymentRouteDescription}</small></div><div><span>Payments</span><strong>{payoutCount}</strong><small>Recorded payout{payoutCount === 1 ? "" : "s"}</small></div></div>
        </section>
      </div>
      <section className="payee-history payee-payment-history"><div className="payee-section-heading"><div><span>Payment history</span><h2>Weekly payouts</h2></div><small>{payoutCount ? `${payoutCount} payment${payoutCount === 1 ? "" : "s"}` : "No payments yet"}</small></div>{payoutCount ? <div className="payee-history-list">{(payouts ?? []).map((item) => { const batch = Array.isArray(item.payout_batches) ? item.payout_batches[0] : item.payout_batches; return <div key={item.id}><div><b>{batch ? `${date(batch.period_start)} – ${date(batch.period_end)}` : "Weekly Villix payout"}</b><span>Scheduled {date(batch?.payout_date ?? null)}</span></div><div><strong>{inr.format(item.payout_amount_minor / 100)}</strong><span className={`payee-payment-status ${item.status}`}>{item.status}</span></div></div>; })}</div> : <div className="payee-empty"><span aria-hidden="true">✓</span><h3>Your payment history starts here</h3><p>Approved weekly payouts will appear here with their amount, date, and status.</p></div>}</section>
