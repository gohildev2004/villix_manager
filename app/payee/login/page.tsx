...
export const metadata: Metadata = { title: "Contributor sign in", robots: { index: false, follow: false } };
...
export default async function PayeeLoginPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  if (user) redirect(contributorPortalPath(host));
  const { error } = await searchParams;
  return <main className="payee-login-page"><section className="payee-login-card">
    <div className="payee-brand"><Image src="/villix-logo.svg" alt="Villix" width={50} height={38} style={{ width: 50, height: 38 }} priority unoptimized /><span>Villix Contributor</span></div>
