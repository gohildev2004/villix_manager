...
type PaymentStatus = "Ready" | "On hold" | "Processing" | "Paid" | "Failed";
...
type PayoutSnapshot = { id: string; exchangeRate: number; adjustmentBps: number; grossInr: number; retainedInr: number; payableInr: number; provider: string; recipients: Array<{ personId: string; status: string; holdReason: string | null; currency: string; amount: number; provider: string | null }> };
...
export default function ManagerApp() {
  ...
  async function dispatchBatch(personIds?: string[]) {
    if (!payoutSnapshot) return;
    const retrying = Boolean(personIds?.some((personId) => paymentStatuses[personId] === "Failed"));
    const message = retrying
      ? "Retry this failed recipient payout? Confirm its latest RazorpayX status first. A new idempotent attempt will be recorded."
      : "Send every ready recipient payout now? Recipients on hold will remain safely outstanding. Real bank transfers cannot be undone.";
    if (!window.confirm(message)) return;
    try {
      const result = await api<{ sent: number; failed: number; held: number }>("/api/payouts/dispatch", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ batchId: payoutSnapshot.id, confirmed: true, personIds }) });
      await refreshState(); notify(`${result.sent} transfer${result.sent === 1 ? "" : "s"} created · ${result.held} on hold`);
...
function SystemHealth({ monitoring, refresh, openView }: { monitoring: OperationalHealth; refresh: () => Promise<void>; openView: (view: View) => void }) {
  ...
  const healthy = monitoring.checks.filter((check) => check.status === "healthy").length;
  return <section className="surface system-health-card">
    <div className="section-header health-page-heading"><div><h2>{monitoring.status === "healthy" ? "All systems operational" : `${issues.length} check${issues.length === 1 ? "" : "s"} need attention`}</h2><p>{monitoring.status === "healthy" ? "No action is needed. Open a check only when you want technical details." : "Open the affected checks for impact details and exact recovery steps."}</p></div><div className="health-heading-actions"><div className={`health-overall ${monitoring.status}`}><span></span>{monitoring.status === "healthy" ? "Healthy" : monitoring.status === "error" ? "Action required" : "Warnings"}</div><button className="health-refresh" onClick={() => void refresh()}>Run checks again</button></div></div>
    <div className="health-meta-line"><span><b>{healthy} of {monitoring.checks.length}</b> checks healthy</span><span>Environment <b>{monitoring.environment}</b></span><span>Updated <b>{checked}</b></span>{monitoring.lastWebhookAt && <span>Webhook <b>{new Intl.DateTimeFormat("en-US", { dateStyle: "medium", timeStyle: "short" }).format(new Date(monitoring.lastWebhookAt))}</b></span>}</div>
...
function HealthCheckCard({ check, openView }: { check: OperationalHealthCheck; openView: (view: View) => void }) {
  ...
  const statusLabel = check.status === "healthy" ? "Healthy" : check.status === "error" ? "Action required" : "Warning";
  return <details className={`health-runbook ${check.status}`} open={check.status !== "healthy"}>
    <summary className="health-runbook-summary">
      <span className="health-dot"></span>
      <span className="health-runbook-copy"><b>{check.label}</b><small>{check.detail}</small></span>
      <span className="health-runbook-state">{statusLabel}</span>
      <span className="health-chevron" aria-hidden="true">⌄</span>
    </summary>
    <div className="health-runbook-body">
...
function Reconciliation({ rows, people, batchStatus, statuses, readyPayments, snapshot, providerReadiness, dispatch, sync }: { rows: PayoutRow[]; people: Person[]; batchStatus: "Draft" | "Approved"; statuses: Record<string, PaymentStatus>; readyPayments: number; snapshot: PayoutSnapshot | null; providerReadiness: ProviderReadiness; dispatch: (personIds?: string[]) => Promise<void>; sync: () => Promise<void> }) {
  ...
  const readyRecipients = snapshot?.recipients.filter((recipient) => statuses[recipient.personId] === "Ready") ?? [];
  const heldRecipients = snapshot?.recipients.filter((recipient) => statuses[recipient.personId] === "On hold") ?? [];
  const failedRecipients = snapshot?.recipients.filter((recipient) => statuses[recipient.personId] === "Failed") ?? [];
  ...
  const canOperate = providerReadiness.payoutsLive && batchStatus === "Approved" && Boolean(snapshot) && providersReady && !hasProcessing;
  const canSendReady = canOperate && readyRecipients.length > 0;
  const recipientCount = snapshot?.recipients.length ?? readyPayments;
  return <div className="stack"><section className="recon-summary"><Stat label="Expected settlement" value={snapshot ? inr.format(snapshot.payableInr) : "Not approved"} note={`${readyRecipients.length} ready · ${heldRecipients.length} on hold · ${recipientCount} total`} /><Stat label="Confirmed INR paid" value={inr.format(paidInr)} note={`${paidRecipients.length} completed · ${failedRecipients.length} failed`} /><Stat label="RazorpayX" value={providersReady ? "Ready" : "Setup needed"} note={providerReadiness.payoutsLive ? "Live Indian bank payouts" : "Test mode · Indian bank payouts only · transfers locked"} /></section>
    {!canOperate && <div className="alert warning-alert"><span>i</span><div><b>{!providerReadiness.payoutsLive ? "Payouts are locked in test mode" : batchStatus === "Draft" ? "Approve and lock this payout first" : hasProcessing ? "Transfers are processing" : "RazorpayX setup is incomplete"}</b><p>{!providerReadiness.payoutsLive ? "You can import receipts, review calculations, configure test beneficiaries, and approve test batches, but no bank transfer can be created." : hasProcessing ? "Do not resend processing transfers. Webhooks update final status; manual sync is available as a fallback." : "Add the signed RazorpayX webhook and server credentials before enabling payouts."}</p></div></div>}
    {heldRecipients.length > 0 && <div className="safety-note"><span>⏸</span><div><b>{heldRecipients.length} recipient payout{heldRecipients.length === 1 ? " is" : "s are"} on hold</b><p>Ready recipients can still be paid. Held amounts remain outstanding until those recipients complete Indian bank onboarding.</p></div></div>}
    <section className="surface table-surface"><div className="section-header"><div><h2>Payment ledger</h2><p>Send ready recipients now; held and failed items remain individually traceable.</p></div><div className="command-actions"><button className="button secondary" disabled={!snapshot || !providerReadiness.razorpayxConfigured} onClick={() => void sync()}>Sync statuses</button><button className="button primary" disabled={!canSendReady} onClick={() => void dispatch()}>{readyRecipients.length ? `Send ${readyRecipients.length} ready payout${readyRecipients.length === 1 ? "" : "s"}` : "No payouts ready"}</button></div></div><div className="data-table recon-table"><div className="data-head"><span>Recipient</span><span>INR instruction</span><span>Provider</span><span>Status</span><span>Readiness</span><span>Action</span></div>{rows.map((row) => { const person = people.find((candidate) => candidate.id === row.key); const recipient = snapshot?.recipients.find((item) => item.personId === row.key); const status = statuses[row.key] || "Ready"; const canRetry = canOperate && status === "Failed" && Boolean(person?.payoutReady); return <div className="data-row" key={row.key}><div className="identity-cell"><Avatar name={row.name}/><div><b>{row.name}</b><small>{row.route}</small></div></div><strong>{recipient ? inr.format(recipient.amount) : money.format(row.payout)}</strong><span>RazorpayX</span><Status value={status}/><div><Status value={status === "On hold" ? "Bank setup needed" : person?.payoutReady ? "Ready" : "Setup needed"}/>{status === "On hold" && recipient?.holdReason && <small className="hold-reason">{recipient.holdReason}</small>}</div><div className="row-actions">{status === "Failed" ? <button disabled={!canRetry} onClick={() => void dispatch([row.key])}>Retry failed</button> : status === "On hold" ? <button disabled>Held</button> : <span className="muted">—</span>}</div></div>; })}</div></section></div>;
}
